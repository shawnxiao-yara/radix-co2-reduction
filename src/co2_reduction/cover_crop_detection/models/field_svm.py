...
class FieldSVM:
    ...
    def load(self) -> None:
        ...
        if (self.model_folder / "cc_field_svm.pickle").is_file():
            with open(self.model_folder / "cc_field_svm.pickle", "rb") as f:
                self.clf = pickle.load(f)  # noqa S301

    def save(self) -> None:
        """Store the classifier and corresponding meta data."""
        assert self.clf is not None
        with open(self.model_folder / "cc_field_svm.pickle", "wb") as f:
