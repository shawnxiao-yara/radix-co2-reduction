...
from src.radix_co2_reduction.cover_crop_detection import FieldSVM, ndvi_feature
...
def get_features(
    ...
    """
    inputs = [(cache_path / f"{c[0]}-{c[1]}", y) for c, y in zip(coordinates, years)]
...
def train(
    ...
    # Predict None-labels
    features = get_features(cache_path, coordinates, years)
...
def infer(
    ...
    return [
        model(load_data(cache_path / f"{c[0]}-{c[1]}"), y)
        for c, y in tqdm(
