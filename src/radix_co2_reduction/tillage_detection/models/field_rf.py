...
from src.radix_co2_reduction.data import BANDS
from src.radix_co2_reduction.tillage_detection.features import custom
from src.radix_co2_reduction.tillage_detection.index import (
    evi,
    ndti,
    ndvi,
    nir,
...
from src.radix_co2_reduction.tillage_detection.models.base import BaseModel
...
class FieldRF(BaseModel):
    ...
    def load(self) -> None:
        ...
        if (self.models_path / "td_field_rf.pickle").is_file():
            with open(self.models_path / "td_field_rf.pickle", "rb") as f:
                self.clf = pickle.load(f)  # noqa S301
        if (self.models_path / "td_field_rf_meta.json").is_file():
            with open(self.models_path / "td_field_rf_meta.json", "r") as f:  # type: ignore
    ...
    def save(self) -> None:
        ...
        assert self.clf is not None
        assert self.feature_mask is not None
        with open(self.models_path / "td_field_rf.pickle", "wb") as f:
            pickle.dump(self.clf, f)
        with open(self.models_path / "td_field_rf_meta.json", "w") as f:  # type: ignore
